...
- FP: 我虽然这里叫它 frame pointer，实际上它根本不是 frame pointer，比如我们当前的 callee 函数是 add，在 add 的代码中引用 FP，该 FP 指向的既不是 callee 的 frame，也不是 caller 的 frame。而是当前函数的输入参数的位置。在输入参数之上，还有 caller 的 return address。
- 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTQxNDU2ODY4Ml19
-->