...
- SP: plan9 的这个 SP 寄存器指向当前栈帧的局部变量的开始位置，使用形如 `symbol+offset(SP)` 的方式，引用函数的输入参数。offset 的合法取值是 [-framesize, 0]，这也是一个词不表意的寄存器。与硬件寄存器 SP 是两个不同的东西。
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE1MDU5ODQxNzBdfQ==
-->