...
- FP: 使用形如 `symbol+offset(FP)` 的方式，可以引用函数的输入参数。例如 `arg0+0(FP)`，`arg1+8(FP)`，如果不加 symbol 时，无法通过编译，虽然在汇编层面来讲，symbol 并没有什么用，加 symbol 主要是为了提升汇编代码的可读性。另外，官方文档虽然这里叫它 frame pointer，实际上它根本不是 frame pointer，比如我们当前的 callee 函数是 add，在 add 的代码中引用 FP，该 FP 指向的既不是 callee 的 frame，也不是 caller 的 frame。而是当前函数的输入参数的位置。在输入参数之上，还有 caller 的 return address。
- 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE4Njc3MjU4MTddfQ==
-->